...
public class CustomerShoppingController implements View.OnClickListener {
  ...
  private void updateQuantity(int updateIndex, int operation, int idCurrentQuantity,
      int idEnteredQuantity) {
    int enteredQuantity = readEnteredQuantity(idEnteredQuantity);
    if (enteredQuantity == -1) {
      Toast.makeText(appContext, "Before Adding/Removing items, Please Enter a quantity!",
