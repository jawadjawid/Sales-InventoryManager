...
public class CustomerHomeController implements View.OnClickListener {
  ...
  private void updateCart() {
    Intent intent = view.getIntent();
    HashMap<Item, Integer> allItems = (HashMap<Item, Integer>) intent
      ...
      public void onClick(DialogInterface dialog, int whichButton) {
        if (edittext.getText().length() == 0) {
          ...
          if (validAccountId(id)) {
            createNewCart(true);
            Toast.makeText(appContext, "Shopping Cart restored from account with id" + id,
                Toast.LENGTH_SHORT).show();
          } else {
            createNewCart(false);
            Toast
                .makeText(appContext, "Cannot Restore as Account was not found", Toast.LENGTH_SHORT)
                .show();
          }
        }
      }
  ...
  private void createNewCart(boolean restore) {
    Customer c = new Customer(recievedUser.getId(), recievedUser.getName(), recievedUser.getAge(),
        recievedUser.getAddress(), recievedUser.getRoleId(), recievedUser.getBalance());
    if (!restore) {
