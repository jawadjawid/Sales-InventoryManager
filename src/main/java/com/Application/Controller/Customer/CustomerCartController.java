...
public class CustomerCartController implements View.OnClickListener {
        ...
        if (recievedCart.checkOut(mydb)) {
          if (accountId != -1) {
            ...
            Toast.makeText(appContext,
                "Cart has been successfully checked out and Account with id " + accountId
                    + " has been deactivated.", Toast.LENGTH_SHORT).show();
          } else {
            Toast
                .makeText(appContext, "Cart has been successfully checked out.", Toast.LENGTH_SHORT)
                .show();
          }
          displayContinueShoppingAlert();
        } else {
          Toast.makeText(appContext,
              "Checkout failed as there is not enough items in the inventory or not enough balance.",
    ...
    alert.setPositiveButton("Save Cart", new DialogInterface.OnClickListener() {
      public void onClick(DialogInterface dialog, int whichButton) {
        if (edittext.getText().length() == 0) {
          Toast.makeText(appContext, "Cannot restore as no Account ID was entered",
              Toast.LENGTH_SHORT).show();
        } else {
          ...
          if (validAccountId(id)) {
            ...
            Toast.makeText(appContext, "Cannot store as Account was not found", Toast.LENGTH_SHORT)
                .show();
          }
        }
      }
      ...
      if (storingAccount.getItemMap().isEmpty()) {
        for (Item i : recievedCart.getItemMap().keySet()) {
          if (recievedCart.getItemMap().get(i) != 0) {
            mydb.insertAccountSummaryH(storingAccount.getId(), i.getId(),
                recievedCart.getItemMap().get(i));
          }
        }
        Toast.makeText(appContext,
            "Shopping Cart stored to account with id" + storingAccount.getId(), Toast.LENGTH_SHORT)
            .show();
      } else {
        Toast.makeText(appContext, "Cannot save to cart as the cart already has items.",
            Toast.LENGTH_SHORT).show();
      }

    } catch (DatabaseInsertException e) {
      Toast.makeText(appContext, "Error with saving cart, please try again later",
          Toast.LENGTH_SHORT).show();
    }
  }
  ...
  private boolean validAccountId(int id) {
    ...
    ArrayList<Account> activeAccounts = helper
        .getUserActiveAccountsH(recievedCart.getCustomer().getId());
      ...
      for (Item i : recievedCart.getItemMap().keySet()) {
        if (recievedCart.getItemMap().get(i) != 0) {
          ...
          if (i.getName().equals("Fishing Rod")) {
            displayItemDetail(R.id.rodLabel, itemsLabel, R.id.RodQuantity,
                recievedCart.getItemMap().get(i));
          } else if (i.getName().equals("Hockey Stick")) {
            displayItemDetail(R.id.stickLabel, itemsLabel, R.id.StickQuantity,
                recievedCart.getItemMap().get(i));
          } else if (i.getName().equals("Skates")) {
            displayItemDetail(R.id.skatesLabel, itemsLabel, R.id.SkatesQuantity,
                recievedCart.getItemMap().get(i));
          } else if (i.getName().equals("Running Shoes")) {
            displayItemDetail(R.id.shoesLabel, itemsLabel, R.id.ShoesQuantity,
                recievedCart.getItemMap().get(i));
          } else if (i.getName().equals("Protein Bar")) {
            displayItemDetail(R.id.barLabel, itemsLabel, R.id.BarQuantity,
                recievedCart.getItemMap().get(i));
          }
        }
      }
  ...
  public void displayTotal() {
    ...
    totalPrice
        .setText("$ " + recievedCart.getTotal().multiply(recievedCart.getTaxRate()).setScale(2));
  }

  public void displayItemDetail(int textViewIdLabel, String itemLabel, int textViewIdQuantity,
      int totalQuantity) {
